...
function renderHomePage() {
  ...
  if (heroWrap) heroWrap.innerHTML = createHeroSVG(true);
...
(function initPetals() {
  ...
  const PETAL_COLORS = ['#FFB7C5','#F4A7B0','#FADADD','#F5F5F0','#FDE8D0','#E8C4D4'];
  const PETAL_COUNT = 18;

  const petals = Array.from({ length: PETAL_COUNT }, () => resetPetal({}));
  ...
  function resetPetal(p) {
    ...
    p.rotationSpeed = (-0.02 + Math.random() * 0.04);
    return p;
  }
